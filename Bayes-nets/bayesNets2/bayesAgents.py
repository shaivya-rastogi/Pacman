...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVars = OBS_VAR_TEMPLATE % obsPos
            newFactor = bn.Factor([obsVars], HOUSE_VARS, bayesNet.variableDomainsDict())
            #pdb.set_trace()
            minDist = float("inf")
            for housePos1 in gameState.getPossibleHouses():
                manhattanDist = util.manhattanDistance(housePos1, obsPos)
                if manhattanDist < minDist:
                    minDist = manhattanDist
                    adjacent = housePos1
            if adjacent == bottomLeftPos:
                adjacent = BOTTOM_LEFT_VAL
            elif adjacent == topLeftPos:
                adjacent = TOP_LEFT_VAL
            elif adjacent == bottomRightPos:
                adjacent = BOTTOM_RIGHT_VAL
            else:
                adjacent = TOP_RIGHT_VAL

            for assignmentDict in newFactor.getAllPossibleAssignmentDicts():
                prob = 0
                if adjacent != assignmentDict[GHOST_HOUSE_VAR] and adjacent != assignmentDict[FOOD_HOUSE_VAR]:
                    if assignmentDict[obsVars] == NO_OBS_VAL:
                        prob = 1
                elif adjacent == assignmentDict[FOOD_HOUSE_VAR] or assignmentDict[FOOD_HOUSE_VAR] == assignmentDict[GHOST_HOUSE_VAR]:
                    if assignmentDict[obsVars] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assignmentDict[obsVars] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                elif adjacent == assignmentDict[GHOST_HOUSE_VAR]:
                    if assignmentDict[obsVars] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignmentDict[obsVars] == BLUE_OBS_VAL:
            ...
            bayesNet.setCPT(obsVars, newFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    newFactor = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    minProb = float("-inf")
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        jointProb = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)

        for currAssignment in jointProb.getAllPossibleAssignmentDicts():
            if currAssignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL:
                if currAssignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                    rightExpectedValue = jointProb.getProbability(currAssignment)

            if currAssignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
                if currAssignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                    leftExpectedValue = jointProb.getProbability(currAssignment)

        leftExpectedValue = leftExpectedValue * WON_GAME_REWARD + (1  - leftExpectedValue) * GHOST_COLLISION_REWARD
        rightExpectedValue = rightExpectedValue * WON_GAME_REWARD + (1 - rightExpectedValue) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue = expectedValue + prob * (max(self.computeEnterValues(explorationEvidence, enterEliminationOrder)))
