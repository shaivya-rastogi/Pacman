...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        evidenceFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for elimination in eliminationOrder:
            evidenceFactors, joinedEvidenceFactor = joinFactorsByVariable(evidenceFactors, elimination)
            if len(joinedEvidenceFactor.unconditionedVariables()) > 1:
                temp_factor = eliminate(joinedEvidenceFactor, elimination)
                evidenceFactors.append(temp_factor)
        final_factor = normalize(joinFactors(evidenceFactors))
        return final_factor
